...
def get_transaction_by_id(acc_token, id):
...
async def get_transaction_by_company(acc_token, company_id):
...
async def get_transactions_by_period(acc_token, start_date, end_date):
...
async def get_turnovers(acc_token):
...
async def get_rate_by_date(acc_token, date):
...
async def calculate_balance_usd(data):
    usd_balance = 0
...
async def get_plot_by_category(data):
    st.title("В разрезе категорий расходов")
    st.write("Графики")
    ...
    st.write("Таблица оборотов")
    st.write(data.drop(columns=columns_to_drop))


async def get_plot_by_companies(data):
    st.title("В разрезе компаний")
    st.write("Графики")
    ...
    # By tl
    trace_product = (px.pie(
        data_for_graph.query('company_id != "Итого"'),
        values='debit_turnover_target_usd',
        names='company_id',
        hole=0.2,
        color_discrete_sequence=px.colors.qualitative.Set1,
